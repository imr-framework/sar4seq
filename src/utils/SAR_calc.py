...
def _load_Q() -> Tuple[np.ndarray, np.ndarray]:
    ...
    path_Q = '/lhome/ext/i3m121/i3m1211/SAR/SAR4seq_python/data/QGlobal.mat'
...
def calc_SAR(file: Union[str, Path, Sequence]) -> None:
    ...
    if tsec[-1] > 10:
        ...
        print(f"Sequence duration: {tsec[-1]:.1f} seconds")
        print(f"Whole Body SAR (10-sec avg) peak: {SAR_wbg_tensec_peak} W/kg (limit: 8 W/kg)")
        print(f"Head SAR (10-sec avg) peak: {SAR_hg_tensec_peak} W/kg (limit: 6.4 W/kg)")
        
        if SAR_wbg_sixmin_peak != 'NA':
            print(f"Whole Body SAR (6-min avg) peak: {SAR_wbg_sixmin_peak} W/kg (limit: 4 W/kg)")
            print(f"Head SAR (6-min avg) peak: {SAR_hg_sixmin_peak} W/kg (limit: 3.2 W/kg)")
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig('/lhome/ext/i3m121/i3m1211/SAR/SAR4seq_python/results/sar_result.png')

if __name__ == "__main__":
    calc_SAR('/lhome/ext/i3m121/i3m1211/SAR/SAR4seq_python/tse_50s.seq')

function [status] = write_qmat(Q,SAR_type,Tissue_types)

switch SAR_type
    
    case 'Global'
         [filename, pathname] = uiputfile('global.qmat', 'Save global Q matrix as');
...
%          write_uppertri(Q.Qemf,fid);
    ...
    case 'Local'
         [tri_filename, tri_pathname] = uiputfile('Local.qmat', 'Save local Q matrix as');
         tri_fid = fopen(fullfile(tri_pathname,tri_filename),'w+','l');
         
         [index_filename, index_pathname] = uiputfile('Local.index', 'Save local Q matrix indices as');
